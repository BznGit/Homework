<template>
  <div class="head">
			<input type="text" class="edit" placeholder="Введите текст для поиска" v-model="search">
			<button v-on:click="isActive=!isActive">{{isActive?'Список':'Таблица'}}</button>
		</div>
		<div class="list" :class={grid:isActive} v-for="article in articles" :key="article">
				<div class="img">
					<a v-bind:href="article.url"><img v-bind:src="article.image"></a>
				</div>
				<div v-show="!isActive" class="descr">
					<h3><a v-bind:href="article.url">{{article.titel}}</a></h3>
...
.head {
			...
			.list {
				...
				border-bottom: 1px solid rgb(70, 6, 219);
				border-radius: 30px;
				width: 100%;
				padding: 2px;
				font-family: Tahoma;
